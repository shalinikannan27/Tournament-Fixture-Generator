...
def generate_excel(data):
    ...
    # ─── HEADER ROWS ──────────────────────────────────────────────────────────
    last_col_idx = 3 + len(round_names)
    ...
    # Column Headers
    header_row = 4
    headers = ["SL NO", "NAME OF THE PLAYER", "INSTITUTION"] + round_names
    ...
    # ─── BODY DATA ────────────────────────────────────────────────────────────
    start_row = 5
    # First, fill the base player data columns (SL, Name, Institution)
    for r_idx, p in enumerate(players):
        ...
        # Institution
        c = ws.cell(row=row_num, column=3, value=p.get('inst'))
        c.alignment = left_align
        c.border = thin_border
    ...
    for c_idx, nm in enumerate(round_names):
        ...
        for r_idx in range(0, num_rows, row_span):
            ...
            # Cap merges to the actual bracket rows so later rounds (SF/Finals)
            # don't extend beyond the fixture table area.
            remaining = num_rows - r_idx
            span = row_span if row_span <= remaining else remaining
            e_row = s_row + span - 1
